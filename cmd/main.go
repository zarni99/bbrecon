...
var (
	...
	enableCloudAssetsShorthand bool
	enableAWS                  bool
	enableAzure                bool
	enableGCP                  bool
	awsRegion                  string
...
func init() {
	...
	flag.BoolVar(&enableCloudAssetsShorthand, "l", false, "Enable cloud asset discovery (all providers)")
	flag.BoolVar(&enableAWS, "aws", false, "Enable AWS asset discovery")
	flag.BoolVar(&enableAzure, "az", false, "Enable Azure asset discovery")
	flag.BoolVar(&enableGCP, "gcp", false, "Enable GCP asset discovery")
	flag.StringVar(&awsRegion, "aws-region", "", "AWS region for asset discovery")
	flag.BoolVar(&enableCloudSummaryOnly, "sum", false, "Show only summary for cloud asset discovery")
	flag.BoolVar(&enableCloudSummaryOnly, "summary-only", false, "Show only summary for cloud asset discovery")
}
...
func main() {
	...
	if debug {
		fmt.Printf("%s Command line arguments: %v\n", info("DEBUG:"), os.Args)
		fmt.Printf("%s Cloud Asset Discovery flag values after parsing:\n", info("DEBUG:"))
		fmt.Printf("%s   enableCloudAssetsShorthand = %v\n", info("DEBUG:"), enableCloudAssetsShorthand)
		fmt.Printf("%s   enableAWS = %v\n", info("DEBUG:"), enableAWS)
		fmt.Printf("%s   enableAzure = %v\n", info("DEBUG:"), enableAzure)
		fmt.Printf("%s   enableGCP = %v\n", info("DEBUG:"), enableGCP)
		fmt.Printf("%s   awsRegion = %s\n", info("DEBUG:"), awsRegion)
	}
	...
	if enableCloudAssetsShorthand || enableAWS || enableAzure || enableGCP {
		if enableSubdomain || enableHTTPProbe || enableDirBrute || enableJSAnalyzer || enableDNSEnum || enableExposureDetection || allModules {
			fmt.Printf("\n%s Cloud Asset Discovery cannot be used with other modules\n", error_("ERROR:"))
			fmt.Printf("Please run Cloud Asset Discovery separately\n")
			fmt.Printf("Example: ./bbrecon -t example.com -l\n")
			os.Exit(1)
		}
	}
	...
	if enableCloudAssetsShorthand || enableAWS || enableAzure || enableGCP {
		...
		if debug {
			fmt.Printf("Cloud Asset Discovery Debug Info:\n")
			fmt.Printf("Flag Values: enableCloudAssetsShorthand=%v, enableAWS=%v, enableAzure=%v, enableGCP=%v\n",
				enableCloudAssetsShorthand, enableAWS, enableAzure, enableGCP)
		}

		if !enableAWS && !enableAzure && !enableGCP {
			...
			if debug {
				fmt.Printf("Enabling all cloud providers\n")
			}
		}
		...
		if awsRegion == "" {
			awsRegion = "us-east-1"
			if debug {
				fmt.Printf("Using default AWS region: %s\n", awsRegion)
			}
		}

		// Handle output directory for cloud module
		var outputDir string
		if outputFile != "" {
			// If user specified an output file, use its directory
		...
		if err != nil {
			fmt.Printf("%s Error initializing cloud asset discovery: %v\n", error_("ERROR:"), err)
		} else {
			findings, err := cloudScanner.ScanAll(context.Background())
			if err != nil {
				fmt.Printf("%s Error in cloud asset discovery: %v\n", error_("ERROR:"), err)
			} else {
				if len(findings) > 0 {
					fmt.Printf("\nSUMMARY: Total cloud assets found: %d\n", len(findings))
				} else {
					fmt.Printf("\nSUMMARY: No cloud assets found\n")
				}
			}
		}
	...
	for _, domain := range domainList {
		...
		if onlyTargetSpecified {
			...
			if !silent && !bruteforceOnlyMode && !enableExposureDetection {
				// Check if any cloud flag is enabled
				if isCloudFlagEnabled() {
					// Disable subdomain enumeration if we're only doing cloud asset discovery
					cfg.SubdomainConfig.Enabled = false
					cfg.HTTPProbeConfig.Enabled = false

					if debug {
						fmt.Printf("%s Cloud asset discovery requested, disabling default subdomain enumeration\n", info("DEBUG:"))
					}
				} else {
					availableSources := subdomain.GetAvailableSources(cfg.SubdomainConfig)
...
func isCloudFlagEnabled() bool {
	return enableCloudAssetsShorthand || enableAWS || enableAzure || enableGCP
}
...
func printHelp() {
	...
	fmt.Println()
	fmt.Printf("  -X\n")
	fmt.Printf("    Enable Exposure Detection module\n")
	fmt.Printf("    %s Detects exposed secrets, API keys, and sensitive files\n", color.HiCyanString("FUNCTION:"))
	fmt.Printf("    %s Cannot be used with other module flags\n", color.HiRedString("RESTRICTION:"))
	fmt.Println()
	fmt.Printf("  -l\n")
	fmt.Printf("    Enable Cloud Asset Discovery module\n")
	fmt.Printf("    %s Discovers cloud resources related to the target domain\n", color.HiCyanString("FUNCTION:"))
	fmt.Printf("    %s Supports AWS, Azure, and GCP resources\n", color.HiCyanString("PLATFORMS:"))
	fmt.Printf("    %s When used alone, enables scanning for all cloud providers\n", color.HiCyanString("DEFAULT:"))
	fmt.Printf("    %s Cannot be combined with other module flags (-S, -H, -D, -J, -N, -X, -A)\n", color.HiRedString("RESTRICTION:"))
	fmt.Println()
	fmt.Printf("  -aws\n")
	fmt.Printf("    Enable AWS asset discovery\n")
	fmt.Printf("    %s Discovers S3 buckets and other AWS resources\n", color.HiCyanString("FUNCTION:"))
	fmt.Printf("    %s Use with -aws-region to specify region (default: us-east-1)\n", color.HiCyanString("USAGE:"))
	fmt.Printf("    %s Can be used independently or with -l flag\n", color.HiCyanString("NOTE:"))
	fmt.Printf("    %s Cannot be combined with non-cloud module flags\n", color.HiRedString("RESTRICTION:"))
	fmt.Println()
	fmt.Printf("  -az\n")
	fmt.Printf("    Enable Azure asset discovery\n")
	fmt.Printf("    %s Discovers Azure Blob storage and other Azure resources\n", color.HiCyanString("FUNCTION:"))
	fmt.Printf("    %s Can be used independently or with -l flag\n", color.HiCyanString("NOTE:"))
	fmt.Printf("    %s Cannot be combined with non-cloud module flags\n", color.HiRedString("RESTRICTION:"))
	fmt.Println()
	fmt.Printf("  -gcp\n")
	fmt.Printf("    Enable GCP asset discovery\n")
	fmt.Printf("    %s Discovers Google Cloud Storage and other GCP resources\n", color.HiCyanString("FUNCTION:"))
	fmt.Printf("    %s Can be used independently or with -l flag\n", color.HiCyanString("NOTE:"))
	fmt.Printf("    %s Cannot be combined with non-cloud module flags\n", color.HiRedString("RESTRICTION:"))
	fmt.Println()
	fmt.Printf("  -aws-region %s\n", color.HiGreenString("string"))
	fmt.Printf("    Specify AWS region for asset discovery\n")
	fmt.Printf("    %s Default region is us-east-1 if not specified\n", color.HiCyanString("DEFAULT:"))
	fmt.Printf("    %s Example: -aws-region eu-west-1\n", color.HiCyanString("EXAMPLE:"))
	fmt.Printf("    %s Only used with -aws or -l flags\n", color.HiCyanString("USAGE:"))
	fmt.Println()
	fmt.Printf("  -sum\n")
	fmt.Printf("    Show only summary of cloud asset discovery results\n")
	fmt.Printf("    %s Provides a concise overview without detailed listings\n", color.HiCyanString("FUNCTION:"))
	fmt.Printf("    %s Use with any cloud discovery flags (-l, -aws, -az, -gcp)\n", color.HiCyanString("USAGE:"))
	fmt.Println()
	fmt.Printf("  -summary-only\n")
	fmt.Printf("    Equivalent to -sum flag (long form)\n")
	fmt.Printf("    %s Provides a concise overview of cloud assets\n", color.HiCyanString("FUNCTION:"))
	fmt.Printf("    %s Use with any cloud discovery flags\n", color.HiCyanString("USAGE:"))
	fmt.Println()
	...
	fmt.Println(color.HiWhiteString(strings.Repeat("_", 50)))
	fmt.Println()
	fmt.Printf("  • To scan all cloud providers: %s\n", color.HiGreenString("./bbrecon -t example.com -l"))
	fmt.Printf("  • To scan only AWS resources: %s\n", color.HiGreenString("./bbrecon -t example.com -aws"))
	fmt.Printf("  • To scan AWS with custom region: %s\n", color.HiGreenString("./bbrecon -t example.com -aws -aws-region eu-west-1"))
	fmt.Printf("  • To scan multiple specific providers: %s\n", color.HiGreenString("./bbrecon -t example.com -aws -az"))
	fmt.Println()
}
...
func validateCommandLineArgs() {
	...
	knownFlags := map[string]bool{
		...
		"-X": true,
		"-l": true, "-ca": true, "-aws": true, "-az": true, "-gcp": true, "-aws-region": true,
		"-sum": true, "-summary-only": true,
	}
	...
	for _, arg := range os.Args {
		if strings.HasPrefix(arg, "-") {
			...
			if flagName == "ca" {
				enableCloudAssetsShorthand = true
				fmt.Printf("%s Warning: '-ca' flag is deprecated, please use '-l' instead\n", warn("DEPRECATED:"))
				continue
			}
			if flagName == "l" {
				enableCloudAssetsShorthand = true
			...
			if strings.HasPrefix(flagName, "aws-region=") {
				awsRegion = strings.Split(flagName, "=")[1]
				continue
			}
		}
	}
