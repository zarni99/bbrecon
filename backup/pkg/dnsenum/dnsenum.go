...
func (e *Enumerator) checkSubdomainTakeover() {
	...
	// Define vulnerable services with verification fingerprints and remediation steps
	...
	for _, record := range e.records {
		...
		for _, service := range vulnerableServices {
			if strings.Contains(record.Value, service.Domain) {
				// Add initial finding based on CNAME pattern match
				...
				if err != nil {
					...
					req, err = http.NewRequest("GET", url, nil)
					if err != nil {
						// If both fail, just add the unverified finding
				...
				if err != nil {
					// If request fails, just add the unverified finding
					foundVulnerabilities = append(foundVulnerabilities, vulnerability)
					continue
				}
				...
				if err != nil {
					// If body read fails, just add the unverified finding
					foundVulnerabilities = append(foundVulnerabilities, vulnerability)
					continue
				}

				// Check if status code matches expected status for takeover
				if resp.StatusCode == service.StatusCode {
					// Further check if fingerprint is in response body
	...
	// Add security issues based on verified findings
