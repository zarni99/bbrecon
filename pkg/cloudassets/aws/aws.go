...
// AWSScanner handles AWS asset discovery
...
func NewAWSScanner(domain string, region string, permutations []string, concurrency int, timeout int, debug bool) *AWSScanner {
	if concurrency <= 0 {
		concurrency = 10
	}
...
// ScanAll runs all AWS scanners
func (a *AWSScanner) ScanAll(ctx context.Context) ([]common.AssetFinding, error) {
	...
	go func() {
		...
		s3Findings, err := DiscoverS3BucketsByPermutation(ctx, a.Permutations, a.Region, a.Concurrency)
		if err != nil && a.Debug {
			fmt.Printf("[DEBUG] Error scanning S3 buckets: %v\n", err)
			mu.Lock()
	...
	// Add more AWS service scanners here
	// ...
	...
	if len(errs) > 0 && a.Debug {
		fmt.Printf("[DEBUG] Completed AWS scanning with %d errors\n", len(errs))
	}

	return findings, nil
}

// PrintFindings prints the AWS findings to the console
func (s *AWSScanner) PrintFindings(findings []common.AssetFinding) {
	if len(findings) == 0 {
	...
	fmt.Printf("\n%s %s %s %s %s\n",
		color.HiCyanString("["),
		color.HiYellowString("AWS ASSETS FOUND"),
		color.HiCyanString("]"),
	...
	if totalItems > totalToShow {
		fmt.Printf("Showing first %d of %d assets (see output file for full list)\n",
			totalToShow,
			totalItems)
	...
	for i := 0; i < itemsToShow; i++ {
		...
		if i > 0 && i%maxItemsPerPage == 0 {
			...
			fmt.Printf("Press Enter to see more results (%d-%d of %d)...",
				i+1,
